...
def combination():
    ...
    start = int(input('Enter the Start Test Frequency: '))
    ...
    if totalsets == 1:
        ...
        for i in setStore:
            ...
            if cc == 0:
                ...
                final = values + frame['0']
        final = (final / totalsets) * scaling
        findValue(final)
        save = str(input('Would you like to save the numerical results? (y/n): [n] '))
        if save == 'y' or save == 'Y':
            outpath = str(input('Enter Path for Oufile: '))
            pd.DataFrame(final).to_csv(outpath + '/TotalCombined.csv')
        else:
            pass
        x = np.arange(start, end, stepsize)
        fig, a = plt.subplots()
